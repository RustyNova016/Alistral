...
impl YimReport {
    pub async fn artist_report(&self) -> String {
        ...
        let stats = self.current.artists_stats().await.unwrap();
        ...
        writeln!(out).unwrap();
        writeln!(out, "Here's the top 10 artists of this year:").unwrap();
    ...
    async fn get_artist_distinct(&self) -> String {
        let current = self.current.artists_stats().await.unwrap();
        let previous = self.previous.artists_stats().await.unwrap();
    ...
    pub async fn top_artists(stats: Vec<ArtistWithRecordings>) -> String {
