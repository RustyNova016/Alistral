...
pub struct LabelWithListens<T: ListenCollectionLike> {
    release_group: Label,
    listens: HashMap<i64, T>,
}

impl<T: ListenCollectionLike> LabelWithListens<T> {
...
impl LabelWithListens<ReleaseWithListens> {
    pub async fn from_listencollection(
        ...
        // Load Releases
        let results = Release::get_label(conn, &recording_refs).await?;
...
impl ListenCollectionLike for LabelWithListens<ReleaseWithListens> {
