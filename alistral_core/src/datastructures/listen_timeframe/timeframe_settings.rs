...
impl TimeframeSettings {
    pub fn get_current_data<T: ExtractTimeframe>(&self, all_time_data: T) -> T {
        all_time_data.extract_timeframe(self.timeframe.start(), self.timeframe.end(), self.include_start, self.include_end)
    }

    pub fn get_previous_data<T: ExtractTimeframe>(&self, all_time_data: T) -> T {
        all_time_data.extract_timeframe(self.timeframe.previous_start(), self.timeframe.start(), self.include_start, !self.include_start)
    }
}